...
from textual.widgets import Header, Footer, Log, Select, Static, Rule, TabbedContent, TabPane, Label, Input, TextArea, Switch
...
class SpeechmaticsConfig(BaseModel):
    api_key: str
    base_connection_url : str = "wss://eu2.rt.speechmatics.com/v2/"

    def get_connection_url(self, language: str) -> str:
        return f"{self.base_connection_url}{language}"
...
class AppSettings(BaseModel):
    speechmatics_config : SpeechmaticsConfig | None
...
class LatecriptApp(App):
            ...
            with TabPane("Transcript", id="transcript_tab"):
                 ...
                 with Horizontal():
                    with Vertical():
                        ...
                        yield TextArea("", id="transcriptwidget", name="Transcript", read_only=True)
                    with Vertical():
                        yield Static("Translation")
                        yield TextArea("", id="translatewidget", name="Translation", read_only=True)
            with TabPane("Settings", id="settings_tab"):        
                with Horizontal(classes="settings_inputs_containers"):
                    ...
                    yield Input(
                        ...
                        value=speechmatics_handler.SPEECHMATICS_API_KEY,
                ...
                with Horizontal(classes="settings_inputs_containers"):        
                    ...
                    yield Select([("fr", 1), ("en", 2)], value=1, id="transcription_language", name="Transcription Language")
                    yield Label("Translation:", classes="label")
                    yield Select([("fr", 1), ("en", 2)], value=2, id="translation_language", name="Translation Language")
                yield Rule()
    ...
    @on(Select.Changed)
    def audio_device_selected(self, event : Select.Changed) -> None:
        self.input_device = event.value
    ...
    def log_in_transcript(self, message: str) -> None:
        """Log message to the transcript widget."""
    ...
    async def handle_transcript_switch(self, event: Switch.Changed) -> None:
        ...
        if event.value:
            ...
            sample_rate = int(pyaudio_handler.get_device_info_by_index(input_device)["defaultSampleRate"])
            import json
            self.update_log(
                json.dumps(
                    obj=pyaudio_handler.get_device_info_by_index(input_device), 
                    indent=4,
                )
            ) 
            ...
            transcript_language = self.query_one("#transcription_language", Select).value
            ...
            try: 
                self.run_worker(
                    self.speechmatics_websocket.run(audio_processor, transcription_config, settings)
                )
            ...
            # User toggled OFF
            self.log_in_transcript("Stopping transcription...")
            # Stop websocket
